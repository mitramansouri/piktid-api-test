...
def process_single_image(PARAM_DICTIONARY, TOKEN_DICTIONARY):
    ...
    j = 0
    result_link = None  # ✅ Define it before returning
    for idx_face in idx_faces:
        try:
            ...
            if response:
                result_link = response  # ✅ Save the download link
        except Exception as inst:
            print(f'type:{type(inst)}, args:{inst.args}, {inst}')
            print('Error in process_single_face')
        j += 1

    return result_link  # ✅ Now it's defined and contains the URL (or None)
